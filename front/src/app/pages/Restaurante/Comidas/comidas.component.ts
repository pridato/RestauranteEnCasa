...
export class ComidasComponent {
  ...
  private loadAndFilterComidas() {
    ...
    $_comidas.subscribe((comidas: IComida[]) => {
  ...
  readQueryParams() {
    this.route.queryParams.subscribe((params) => {
      ...
      if (params["filter"]) {
        ...
        this.rest
          .filterByCategory(params["filter"])
          .subscribe((comidas: IComida[]) => {
            this.comidas = comidas;
          });
      }
    });
  ...
  filtrarComidas() {
    ...
    this.busquedaSvc.getTextoABuscar().subscribe((texto) => {
      if (texto.length > 0) {
        this.comidas = this.comidas.filter((comida) =>
          comida.nombre.toUpperCase().includes(texto.toUpperCase())
        );
      } else {

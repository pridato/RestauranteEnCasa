...
export class CamareroDashboardComponent {
  ...
  constructor(
    private camareroSvc: CamareroDashboardService,
  ...
  getPedidos() {
    this.camareroSvc.cargarPedidos().subscribe((pedidos) => {
      ...
      this.pedidos.forEach((pedido) => {
        ...
        $user.subscribe(
          (data: ICliente) => (pedido.nombreUsuario = data.nombre)
        );
  ...
  marcarPedidoCompletado(mesa: number) {
    ...
    this.camareroSvc.actualizarPedido(pedido!).subscribe((data) => {
